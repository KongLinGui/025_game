...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class CameraBehavior : MonoBehaviour
	{
	    ...
	    protected Vector3 _minimumZoomOffset;
	    protected Vector3 _maximumZoomOffset;
		...
		protected virtual void Initialize()
		{
	        ...
	        // we determine the min and max zoom offsets
	        _minimumZoomOffset = MinimumZoom - _playerPosition;
	        _maximumZoomOffset = MaximumZoom - _playerPosition;
	    ...
	    protected virtual void FixedUpdate()
		{
	        ...
	        { 
	            ...
	            _playerPosition = GameManager.Instance.CurrentPlayableCharacters[0].transform.position;
	        }
	        // if the camera's not supposed to follow the player on one of its axis, we reset these values.
	        if (!FollowsPlayerX) { _playerPosition.x = 0; _minimumZoomOffset.x = MinimumZoom.x; _maximumZoomOffset.x = MaximumZoom.x; }
	        if (!FollowsPlayerY) { _playerPosition.y = 0; _minimumZoomOffset.y = MinimumZoom.y; _maximumZoomOffset.y = MaximumZoom.y; }
	        if (!FollowsPlayerZ) { _playerPosition.z = 0; _minimumZoomOffset.z = MinimumZoom.z; _maximumZoomOffset.z = MaximumZoom.z; }
	        ...
	        _newCameraPosition = Vector3.Lerp(transform.position, _maximumZoomOffset + _playerPosition, CameraSpeed*Time.fixedDeltaTime);
	        ...
	        if (_shakeDuration > 0)
	        {
	            ...
	            _shakeDuration -= _shakeDecay * Time.fixedDeltaTime;
	        }
	    ...
	    {
	    	// the shake method just sets a few variables that will be used in the FixedUpdate(), actually shaking the camera.
